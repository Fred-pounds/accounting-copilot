...
const s: Record<string, React.CSSProperties> = {
  ...
  refreshBtn: {
    width: 38, height: 38, borderRadius: 9, border: '1.5px solid var(--color-border)',
    backgroundColor: 'var(--color-card)', color: 'var(--color-text-secondary)',
    display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
    transition: 'all var(--transition-fast)',
  },
  cardsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 16, marginBottom: 20 },
  card: {
    position: 'relative', backgroundColor: 'var(--color-card)', borderRadius: 14,
    padding: '20px 22px 18px', boxShadow: 'var(--shadow-card)', overflow: 'hidden',
    animation: 'slideUp 0.4s ease-out both',
    border: '1px solid rgba(0,0,0,0.04)',
    transition: 'box-shadow var(--transition-fast), transform var(--transition-fast)',
  },
  cardTop: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 },
  cardLabel: { fontSize: '0.7rem', fontWeight: 700, color: 'var(--color-text-muted)', textTransform: 'uppercase' as const, letterSpacing: '0.8px' },
  iconWrap: { width: 38, height: 38, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  cardValue: { fontSize: '1.75rem', fontWeight: 800, letterSpacing: '-0.5px', lineHeight: 1.15 },
  ...
  approvalBanner: {
    ...
    marginBottom: 20, textDecoration: 'none', color: '#92400e',
    transition: 'background-color var(--transition-fast)',
  },
  approvalIcon: {
    width: 36, height: 36, borderRadius: 9, backgroundColor: '#fef3c7',
    display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#d97706', flexShrink: 0,
  },
  chartsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(440px,1fr))', gap: 16 },
  chartCard: {
    backgroundColor: 'var(--color-card)', padding: '20px 22px',
  ...
  chartTitle: { fontSize: '0.95rem', fontWeight: 700, color: 'var(--color-text)' },
  chartSub: { fontSize: '0.75rem', color: 'var(--color-text-muted)', marginTop: 2 },
  legendRow: { display: 'flex', alignItems: 'center', fontSize: '0.75rem', color: 'var(--color-text-secondary)' },
  dot: { display: 'inline-block', width: 8, height: 8, borderRadius: '50%', marginRight: 5 },
  badge: {
    fontSize: '0.72rem', color: 'var(--color-text-muted)', backgroundColor: 'var(--color-surface)',
    padding: '3px 10px', borderRadius: 'var(--radius-full)', border: '1px solid var(--color-border)',
  },
  errorCard: {
